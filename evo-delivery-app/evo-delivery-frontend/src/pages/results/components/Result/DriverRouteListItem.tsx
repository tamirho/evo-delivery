...
export const DriverRouteListItem = ({
  ...
  return (
    <>
      <ListItem key={route.driver._id} disablePadding alignItems='center'>
        {hasLoadConstraintViolated || hasTravelDistanceConstraintViolated ? (
        ...
        <ListItemButton
          onClick={() => {
            if (openCollapseItemKey !== route.driver._id) setOpenCollapseItemKey(route.driver._id as string);
            else setOpenCollapseItemKey(null);
          }}
        >
          <ListItemAvatar>
            <DriverAvatar  name={route.driver.name} color={routeColor}/>
          </ListItemAvatar>
          <ListItemText
            ...
            secondary={
              <>
                ...
                <Typography
                  ...
                  component='span'
                  variant='body2'
                  color={hasTravelDistanceConstraintViolated ? 'error' : 'text.secondary'}
                >
                  {`Planned travel distance: ${route.totalDistance.toFixed(2)} KMs`}
                </Typography>
                <br />
                <br />
                <Typography sx={{ display: 'inline' }} component='span' variant='body2' color='text.secondary'>
                ...
                <Typography
                  ...
                  component='span'
                  variant='body2'
                  color={hasLoadConstraintViolated ? 'error' : 'text.secondary'}
                >
                  {`Planned travel load: ${route.load.toFixed(2)}%`}
                </Typography>
              </>
            }
          />
